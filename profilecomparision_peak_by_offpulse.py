...
def fft_shift(prf,d):
    ...
    fft_len = len(prf_fft) 
    prf_fft_shifted = np.exp(-1j*2*np.pi*np.arange(fft_len)*d) * prf_fft 
    prf_shifted = fft.irfft(prf_fft_shifted) 
    return prf_shifted 

def prf_shift_scale(prf, d, a, b): 
    prf_shifted = fft_shift(prf, d) 
...
def prf_diff_fn(prf1, prf2):
    def prf_diff(params):
        ...
        a = params['a']
        b = params['b']
        prf2_shifted_scaled = prf_shift_scale(prf2, d, a, b)
        return prf1 - prf2_shifted_scaled
        
    return prf_diff
    
def guess_align_params(prf1, prf2): 
    del0 = (np.argmax(prf1) - np.argmax(prf2))/len(prf2) 
...
def prf_align(prf1, prf2):
    ...
    del0,a0,b0 = guess_align_params(prf1, prf2)
    params = lmfit.Parameters()
    params.add('d', value=del0)
    ...
    return prf_shift_scale(prf2, d, a, b) 
...
if __name__=="__main__":
    ...
    for prf1file in prffiles:
        ...
        off_pulse = np.zeros(10)
        off_pulse[:10] = prf1a[:10]
        op_rms = np.var(off_pulse)

        if ((np.max(diff)/op_rms)<1.5):
           print("Test passed")
        else:
           print("Test Failed")
        # prf_as.append(prf1a)
...
#python3 profilecomparision.py J1939+2134_59208.481806_500.rfiClean.col.b1r0.fits.txt J1939+2134_59208.481806_500.rfiClean.col.b1r1.fits.txt
#python3 profilecomparision.py J1939+2134_59208.481806_500.rfiClean.col.b1r1.fits.txt J1939+2134_59208.481806_500.rfiClean.col.nocfg.fits.txt
